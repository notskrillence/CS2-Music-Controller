...
QMainWindow, QWidget#Root, QWidget#Page {{ background: {p.background}; }}
...
QLabel#WindowMark {{ color: {p.on_primary}; background: {p.primary}; border-radius: 8px; font-size: 11px; font-weight: 800; padding: 4px 7px; }}
QToolButton#WindowButton {{ background: transparent; border: 0; border-radius: 8px; min-width: 34px; min-height: 30px; color: {p.text_muted}; }}
QToolButton#WindowButton:hover {{ background: {p.surface_high}; color: {p.text}; }}
QToolButton#CloseButton {{ background: transparent; border: 0; border-radius: 8px; min-width: 34px; min-height: 30px; color: {p.text_muted}; }}
QToolButton#CloseButton:hover {{ background: #c42b3a; color: #ffffff; }}
...
QPushButton#NavButton {{
    ...
    color: {p.text_muted}; background: transparent; font-weight: 550;
}}
QPushButton#NavButton:hover {{ background: {p.surface_high}; color: {p.text}; }}
QPushButton#NavButton:checked {{ background: {p.primary_container}; color: {p.text}; font-weight: 650; }}
QLabel#PageTitle {{ color: {p.text}; font-size: 25px; font-weight: 650; }}
...
QLabel#Kicker {{ color: {p.text_muted}; font-size: 11px; font-weight: 650; }}
QLabel#HeroState {{ color: {p.text}; font-size: 39px; font-weight: 650; }}
QLabel#MetricValue {{ color: {p.text}; font-size: 22px; font-weight: 650; }}
QLabel#MetricLabel {{ color: {p.text_muted}; font-size: 12px; }}
QLabel#SectionTitle {{ color: {p.text}; font-size: 15px; font-weight: 620; }}
QLabel#Muted {{ color: {p.text_muted}; }}
QLabel#Faint {{ color: {p.text_faint}; font-size: 12px; }}
QLabel#Success {{ color: {p.success}; font-weight: 650; }}
QLabel#Warning {{ color: {p.warning}; font-weight: 650; }}
QLabel#Danger {{ color: {p.danger}; font-weight: 650; }}
...
QLabel#AlbumArt {{ background: {p.surface_low}; border: 1px solid {mix(p.outline, p.primary, 0.25)}; border-radius: {radius}px; }}
QLabel#AlbumPlaceholder {{ color: {p.primary}; background: {p.surface_low}; border: 1px solid {p.outline}; border-radius: {radius}px; font-size: 25px; font-weight: 500; }}
QPushButton, QToolButton {{
    color: {p.text}; background: {p.surface_high}; border: 1px solid {p.outline};
    border-radius: {small}px; padding: 8px 13px; font-weight: 580;
}}
...
QPushButton#Primary {{ background: {p.primary}; border-color: {p.primary}; color: {p.on_primary}; font-weight: 650; }}
...
QLineEdit, QComboBox {{
    ...
    border-radius: {small}px; padding: 8px 10px; min-height: 20px;
}}
...
QSlider::groove:horizontal {{ height: 5px; background: {p.surface_highest}; border-radius: 2px; }}
QSlider::sub-page:horizontal {{ background: {p.primary}; border-radius: 2px; }}
QSlider::handle:horizontal {{ width: 16px; margin: -6px 0; border-radius: 8px; background: {p.primary}; border: 2px solid {p.text}; }}
...
QProgressBar {{ background: {p.surface_highest}; border: 0; border-radius: 4px; min-height: 8px; max-height: 8px; text-align: center; }}
QProgressBar::chunk {{ background: {p.primary}; border-radius: 4px; }}
QScrollArea {{ border: 0; background: transparent; }}
QScrollArea > QWidget > QWidget {{ background: transparent; }}
QScrollBar:vertical {{ background: transparent; width: 10px; margin: 2px; }}
QScrollBar::handle:vertical {{ background: {p.outline_strong}; border-radius: 4px; min-height: 28px; }}
...
QLabel#ProfileBadge {{ color: {p.on_primary}; background: {p.primary}; border-radius: 9px; padding: 3px 8px; font-size: 11px; font-weight: 650; }}
